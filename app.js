...
const siteCodeFilter = document.querySelector("#siteCodeFilter");
...
try {
  adminPassword.value = sessionStorage.getItem("devicePanelAdminPassword") || "";
} catch (error) {
  /* ignore storage errors */
}
...
adminPassword.addEventListener("input", () => {
  try {
    sessionStorage.setItem("devicePanelAdminPassword", adminPassword.value);
  } catch (error) {
    /* ignore storage errors */
  }
});
...
function getFilteredDevices() {
  ...
  const siteCode = siteCodeFilter.value.trim().toLowerCase();
  const number = numberFilter.value.trim().toLowerCase();
  ...
  const filtered = devices.filter((device) => {
    ...
    return (!query || haystack.includes(query)) &&
      ...
      (!siteCode || String(device.areaCode || "").toLowerCase().includes(siteCode)) &&
      (!number || String(device.number || "").toLowerCase().includes(number)) &&
...
function row(device) {
  ...
  const detailFields = [
    ...
    ["Zakład", `${device.site || ""} ${device.areaCode ? `(${device.areaCode})` : ""}`.trim()],
    ["Numer", device.number || ""],
...
async function startSshTunnel(name) {
  ...
  if (!password) {
    alert("Wpisz hasło admina, aby uruchomić tunel SSH.");
    adminPassword.focus();
    return;
  }
...
async function saveTunnelSettings() {
  const password = adminPassword.value;
  if (!password) {
    alert("Wpisz hasło admina, aby zapisać ustawienia tunelu.");
    adminPassword.focus();
    return;
  }
...
function deleteEditedDevice() {
  ...
  const password = adminPassword.value;
  if (!password) {
    alert("Wpisz hasło admina przed usunięciem.");
    adminPassword.focus();
    return;
  }
...
siteCodeFilter.addEventListener("input", resetPageAndRender);
numberFilter.addEventListener("input", resetPageAndRender);
